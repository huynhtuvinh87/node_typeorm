...
export class MedicineController {
  ...
  @Post('type/add')
  // @Middleware([checkRole([{ role: Roles.ADMIN }])])
  @Middleware([])
  private async addType(req: Request, res: Response, next: NextFunction): Promise<void> {
    Log.info(this.className, 'addType', `RQ`, { req: req });
    ...
    try {
      ...
      const newResult: User[] = result.filter(user => this.calcCrow(user.address.lat, user.address.log, parseFloat(lat.toString()), parseFloat(long.toString())) <= (parseInt(range.toString()) / 1000));
  ...
  private async addUser(req: Request, res: Response, next: NextFunction,): Promise<void> {
    ...
    try {
      ...
      const result: User = await this.userService.findByUserName(user.username).catch((e) => {
        throw e;
      });
  ...
  private async updateUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      const newUser: User = await this.userService.update(user.username, user).catch((e) => {
        throw e;
      });
