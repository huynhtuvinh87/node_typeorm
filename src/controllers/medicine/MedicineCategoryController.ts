import { NextFunction, Request, Response } from 'express';
import { Middleware, Controller, Post, Get, Put, Delete } from '@overnightjs/core';
import { MedicineCategoryService } from '../../services/medicine/index';
import { MedicineCategory } from '../../entities/medicine/MedicineCategory';
import { Service } from 'typedi';
import { validate } from "class-validator"
import { checkJwt } from '../../middleware/checkJwt.middleware';
import { default as slugify } from 'slugify';
import { BaseResponse } from '../../services/BaseResponse';
@Service()
@Controller('api/medicine-category')
export class MedicineCategoryController {
  private dataResponse: BaseResponse = new BaseResponse();
  constructor(private readonly medicineCategoryService: MedicineCategoryService) { }

  @Get('list')
  @Middleware([checkJwt])
  private async list(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {

      const medicineCategory = await this.medicineCategoryService.index({
        relations: ['medicineType'],
        where: req.query.status ? { status: req.query.status} : {},
      }).catch((e) => {
        throw e;
      });

      this.dataResponse.data = { result: medicineCategory}
      res.status(200).json(this.dataResponse);
    } catch (e) {
      next(e);
    }
  }

  
  @Get('app/list')
  private async appList(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {

      const medicineCategory = await this.medicineCategoryService.index({
        relations: ['medicineType'],
        where: req.query.status ? { status: req.query.status} : {},
      }).catch((e) => {
        throw e;
      });

      this.dataResponse.data = { result: medicineCategory}
      res.status(200).json(this.dataResponse);
    } catch (e) {
      next(e);
    }
  }

  @Post('create')
  @Middleware([checkJwt])
  private async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const medicineCategory: MedicineCategory = <MedicineCategory>req.body;
      const errors = await validate(medicineCategory);
      if (errors.length > 0) {
        this.dataResponse.success = false;
        res.status(200).json(this.dataResponse);
        return;
      }
      medicineCategory.slug = slugify(req.body.title, '-');
      const create = await this.medicineCategoryService.store(medicineCategory).catch((e) => {
        throw e;
      });

      this.dataResponse.data = { result: create}
      res.status(200).json(this.dataResponse);
    } catch (e) {
      next(e);
    }
  }

  @Put('update/:id')
  @Middleware([checkJwt])
  private async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const medicineCategory: MedicineCategory = <MedicineCategory>req.body;
      const errors = await validate(medicineCategory)
      if (errors.length > 0) {
        this.dataResponse.success = false;
        res.status(200).json(this.dataResponse);
        return;
      }
      medicineCategory.slug = slugify(req.body.title);
      const update = await this.medicineCategoryService.update(req.params.id, medicineCategory).catch((e) => {
        throw e;
      });

      this.dataResponse.data = { result: update}
      res.status(200).json(this.dataResponse);
    } catch (e) {
      next(e);
    }
  }

  @Delete('delete/:id')
  @Middleware([checkJwt])
  private async delete(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const del = await this.medicineCategoryService.delete(req.params.id).catch((e) => {
        throw e;
      });

      this.dataResponse.data = { result: del}
      res.status(200).json(this.dataResponse);
    } catch (e) {
      next(e);
    }
  }
}
