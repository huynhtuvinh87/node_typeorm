...
export class ArticleController {
  ...
  private async list(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const [result, total] = await this.articleService.paginate({
        select: ['id', 'title', 'description', 'content', 'photo', 'status', 'category', 'createdAt'],
        relations: ['category'],
  ...
  private async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const article: Article = <Article>req.body;
  ...
  private async delete(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const del = await this.articleService.delete(req.params.id).catch((e) => {
        throw e;
      });
